...
internal class RequestQueue : IDisposable, IAsyncDisposable
{
    ...
    internal async Task EnterGlobalAsync(int id, RestRequest request)
    {
        ...
        if (millis > 0)
        {
            QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Sleeping {millis} ms (Pre-emptive) [Global]");
    ...
    internal (RequestBucket?, BucketId?) UpdateBucketHash(BucketId id, string QQBotHash)
    {
        if (!id.IsHashBucket)
        {
            BucketId bucket = BucketId.Create(QQBotHash, id);
