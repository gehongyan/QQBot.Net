...
internal class RequestBucket
{
    ...
    public async Task<Stream> SendAsync(RestRequest request)
    {
        ...
        QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Start");
        LastAttemptAt = DateTimeOffset.UtcNow;
        while (true)
        {
            ...
            QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Sending...");
                ...
                if (response.StatusCode is not (HttpStatusCode.OK or HttpStatusCode.Accepted or HttpStatusCode.NoContent))
                    switch (response.StatusCode)
                    {
                        case (HttpStatusCode)429:
                            ...
                            {
                                QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] (!) 429");
                            }
            ...
            finally
            {
                ...
                QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Stop");
            }
        }
    }
    ...
    {
        QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Trigger Bucket");
        await EnterAsync(id, request).ConfigureAwait(false);
        UpdateRateLimit(id, request, default, false);
    }
        ...
        while (true)
        {
            ...
            if (windowCount >= 0 && semaphore < 0)
            {
                if (!isRateLimited)
                {
                    ...
                    if (ignoreRatelimit)
                    {
                        QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Ignoring ratelimit");
                        break;
                    }
                }
                ...
                if (resetAt.HasValue && resetAt > DateTimeOffset.UtcNow)
                {
                    ...
                    QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Sleeping {millis} ms (Pre-emptive)");
                ...
                else
                {
                    if ((timeoutAt - DateTimeOffset.UtcNow)?.TotalMilliseconds < MinimumSleepTimeMs)
                        ThrowRetryLimit(request);
                    QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Sleeping {MinimumSleepTimeMs}* ms (Pre-emptive)");
            ...
            QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Entered Semaphore ({semaphore}/{WindowCount} remaining)");
            break;
        }
    }
    ...
    private void UpdateRateLimit(int id, IRequest request, RateLimitInfo info, bool is429, bool redirected = false)
    {
        ...
        lock (_lock)
        {
            QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Raw RateLimitInto: IsGlobal: {info.IsGlobal}, Limit: {info.Limit}, Remaining: {info.Remaining}, ResetAfter: {info.ResetAfter?.TotalSeconds}");
            if (redirected)
            {
                ...
                QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Decrease Semaphore");
            }
            ...
            if (info.Bucket != null && !redirected)
            {
                ...
                if (hashBucket.Item1 is not null && hashBucket.Item2 is not null)
                {
                    ...
                    {
                        Id = hashBucket.Item2;
                        QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Promoted to Hash Bucket ({hashBucket.Item2})");
                    }
                    else
                    {
                        ...
                        QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Redirected to {_redirectBucket.Id}");
                        return;
                    }
                }
            }
            ...
            if (info.Limit.HasValue && WindowCount != info.Limit.Value)
            {
                ...
                QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Updated Limit to {WindowCount}");
            }

            if (info.Remaining.HasValue && _semaphore != info.Remaining.Value)
            {
                _semaphore = info.Remaining.Value;
                QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Updated Semaphore (Remaining) to {_semaphore}");
            }
            ...
            if (is429)
            {
                ...
                QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Reset-After: {info.ResetAfter} ({info.ResetAfter?.TotalMilliseconds} ms)");
            }
            ...
            else if (info.ResetAfter.HasValue) // && (request.Options.UseSystemClock.HasValue && !request.Options.UseSystemClock.Value)
            {
                ...
                QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Reset-After: {info.ResetAfter.Value} ({info.ResetAfter?.TotalMilliseconds} ms)");
            }
            ...
            else if (request.Options.IsClientBucket && Id != null)
            {
                ...
                QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Client Bucket ({ClientBucket.Get(Id).WindowSeconds * 1000} ms)");
            }
            else if (request.Options.IsGatewayBucket && request.Options.BucketId != null)
            {
                ...
                QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Gateway Bucket ({GatewayBucket.Get(request.Options.BucketId).WindowSeconds * 1000} ms)");
                if (!hasQueuedReset)
                {
                    _resetTick = resetTick;
                    LastAttemptAt = resetTick.Value;
                    QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Reset in {(int)Math.Ceiling((resetTick - DateTimeOffset.UtcNow).Value.TotalMilliseconds)} ms");
            ...
            if (resetTick == null)
            {
                ...
                QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] Disabled Semaphore");
                return;
            }
    ...
    private async Task QueueReset(int id, int millis, IRequest request)
    {
        ...
        while (true)
        {
            ...
            lock (_lock)
            {
                ...
                {
                    QQBotDebugger.DebugRatelimit($"[Ratelimit] [{id}] * Reset *");
                    _semaphore = WindowCount;
                    _resetTick = null;
                    return;
                }
