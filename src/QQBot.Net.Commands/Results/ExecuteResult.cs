...
public struct ExecuteResult : IResult
{
    ...
    ///     <see cref="QQBot.Commands.CommandError.Exception"/>，原因为异常消息。
    /// </returns>
    public static ExecuteResult FromError(Exception? ex) => new(ex, CommandError.Exception, ex?.Message);

    /// <summary>
    ///     初始化一个包含指定结果的 <see cref="ExecuteResult"/> 结构的新实例，根据指定的 <see cref="QQBot.Commands.IResult.Error"/> 和
    ///     <see cref="QQBot.Commands.IResult.ErrorReason"/>，这可能是一个成功的执行，也可能是一个失败的执行。
    /// </summary>
