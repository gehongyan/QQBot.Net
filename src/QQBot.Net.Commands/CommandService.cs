...
///     <see cref="QQBot.Commands.ModuleBase"/> 或 <see cref="QQBot.Commands.Builders.ModuleBuilder"/>。 <br />
///     此服务还提供了几个事件，用于监视命令的使用情况；例如 <see cref="QQBot.Commands.CommandService.Log" /> 用于任何与命令相关的日志事件，
/// </remarks>
public class CommandService : IDisposable
{
    ...
    ///     事件参数：
    ///     <list type="number">
    ///     <item> <see cref="QQBot.LogMessage"/> 参数是日志消息。 </item>
    ///     </list>
    /// </remarks>
    ...
    ///     事件参数：
    ///     <list type="number">
    ///     <item> <see cref="QQBot.Commands.CommandInfo"/>? 参数是执行的命令，如果命令在解析或先决条件阶段失败，则可能为 <c>null</c>。 </item>
    ///     <item> <see cref="QQBot.Commands.ICommandContext"/> 参数是命令的上下文。 </item>
    ///     <item> <see cref="QQBot.Commands.IResult"/> 参数是命令的结果。 </item>
    ///     </list>
    /// </remarks>
    ...
    /// <exception cref="InvalidOperationException"> 默认运行模式不能设置为 <see cref="QQBot.Commands.RunMode.Default"/>。 </exception>
    ...
    /// <exception cref="InvalidOperationException"> 无法构建 <see cref="QQBot.Commands.ModuleInfo"/>；可能提供了无效的类型。 </exception>
    /// <example>
    ...
    {
        string input = context.Message.Content[argPos..];
        ...
        MultiMatchHandling multiMatchHandling = MultiMatchHandling.Exception)
    {
        ...
        Dictionary<CommandMatch, PreconditionResult> preconditionResults = [];
