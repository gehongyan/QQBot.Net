...
/// <summary>
///     标记指定的参数应有应由指定的 <see cref="QQBot.Commands.TypeReader"/> 读取并解析。
/// </summary>
/// <remarks>
///     此特性允许在解析命令参数时为指定的参数指定特定的
///     <see cref="QQBot.Commands.TypeReader"/>，可用于在不影响其他命令的情况下使用特定的命令解析器。
///     <br />
///     <note type="warning">
///         标记此特性的类型解析器类型必须继承自 <see cref="QQBot.Commands.TypeReader"/>，否则将引发
...
public sealed class OverrideTypeReaderAttribute : Attribute
{
    ...
    /// <exception cref="InvalidOperationException"> 所提供的类型解析器类型不是 <see cref="QQBot.Commands.TypeReader"/> 的派生类。 </exception>
